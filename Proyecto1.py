...
def codigo_ciudad(ciudad):                    #Devuelve el valor numerico de la ciudada dado el nombre
    print("codigo ciudas")
    if ciudad== "Poza Rica": 
        return 0
    if ciudad== "Veracruz":
...
class Nodo(object):
    ...
    def __init__(self,estado,padre,hdlr_idx):
        super(Nodo, self).__init__()
        self.estado = estado 
        self.nodo_padre= padre
        self.gn = 0 #camino del estado inicial al nodo
        self.hn = hdlr_fin[hdlr_idx] #num de pasos a lo largo del camino desde el estado inicial
...
class Agente_viajero(object):
    ...
    def extraer_nodo_menor(self,lista):            
        menor_fn=calcular_fn(lista[0])
        for x in range(len(lista)):
            if calcular_fn(lista[x])<menor_fn:
                menor_fn=lista[x]
                   

    #A-Estrella (G, nodo-Inicial, nodo-Final) {Precondición Q1: G = (V, E) ∧ nodo-Inicial, nodo-Final ∈V)         
    def A_estrella(self,G,ciudad_inicial,cod_cd1,ciudad_final,cod_cd2):
        #1: Disponibles: = φ
        #2: Analizados: = φ
        print("a estrella")
        OPEN_list = list()
        ...
        nodo_actual=self.configurar_nodo(nodo_inicial,0) 
        #4: Nodo-Actual: = Asignar-Padre (nodo-Inicial, nodo-Inicial, Nodo-Actual) 
        nodo_actual= self.asignar_padre(nodo_inicial,nodo_actual)
        ...
        while condition:
            ...
            nodo_actual=self.extraer_nodo_menor()
...
if __name__ == "__main__":
    ...
    print(codigo_ciudad)
    codigo_ciudad_ori=codigo_ciudad(ciudad_ori)
    print(codigo_ciudad)                    
    ...
    agente.A_estrella(mapa,ciudad_ori,ciudad_dest,codigo_ciudad)
    '''                                                    
    g_busca = {
        'A': ['B', 'C'],
        'B': ['D', 'E'],
